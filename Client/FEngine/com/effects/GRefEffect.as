package com.effects {
	...
	public class GRefEffect extends GEffect {
		...
		private var BNeedToChangeTarget : Boolean = false;
		
		override protected function onChangeTarget() : void {
			if ( _target.parent == null && !BNeedToChangeTarget)
			{
				BNeedToChangeTarget = true;
				return;
			}
			BNeedToChangeTarget = false;
			if(_ref_bp == null) {
				_ref_bp = new Bitmap();
		...
		override protected function timerHandler(event : TimerEvent) : void {
			if ( BNeedToChangeTarget )
			{
				onChangeTarget();
			}
			var bd : BitmapData = BDUtil.BitmapCreator(_target.width, _target.height, true, 0);
			...
			var mask_bd : BitmapData = BDUtil.BitmapCreator(width, height, true, 0);
			mask_bd.draw(shape);
