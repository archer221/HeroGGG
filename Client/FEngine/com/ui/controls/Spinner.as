package com.ui.controls {
	...
	public class Spinner extends UIComponent {
		...
		protected function initEvents() : void {
			...
			_textInput.addEventListener(TextInput._textinputEventChange, textinput_textchangeHandler);
		}
		protected function textinput_textchangeHandler(e:Event):void {
			var idx : int = 0;
			for ( ; idx < _model.size; idx++ )
			{
				var tlabel :LabelSource = _model.getAt(idx) as LabelSource;
				if ( tlabel != null && tlabel.text == _textInput.textField.text )
				{
					break;
				}
			}
			if ( idx >= 0 && idx < _model.size ) {
				_selectionModel.index = idx;
			}
		}
